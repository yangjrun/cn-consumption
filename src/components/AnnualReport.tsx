  ...
  return (
    <div className="annual-report-page">
      ...
      <article className="statement" ref={reportRef}>
        ...
        <section className="statement__section"><div className="statement__section-head"><div><span>MONTH BY MONTH</span><h2>12 个月税费明细</h2></div><p>合计 {money(result.monthlySchedule.reduce((sum, item) => sum + item.total, 0))}</p></div><div className="statement-months">{result.monthlySchedule.map((point) => <div key={point.month} className={point.eventIds.length ? 'has-event' : ''}><span>{point.month} 月</span><strong>{money(point.total)}</strong><small>工资税 {money(point.salaryIncomeTax + point.bonusTax)}</small>{point.eventIds.length > 0 && <b>{point.eventIds.length} 个事件</b>}</div>)}</div></section>
