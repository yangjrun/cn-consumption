    ...
    <div className="ledger-chart-block">
      ...
      <div className="ledger-chart" role="list" aria-label="逐月可识别税费">
        {schedule.map((point, index) => {
          ...
          const socialChanged = previous && Math.abs(point.socialBase - previous.socialBase) > .01
          return (
            <div className="ledger-month" key={point.month} role="listitem" aria-label={`${point.month} 月，可识别税费 ${money(point.total)}`}>
              ...
              <div className="ledger-month__markers" aria-hidden="true">
                ...
                {socialChanged && <i className="is-social" title="社保基数变化" />}
      ...
      <div className="ledger-legend">{visibleSegments.map((segment) => <span key={segment.key}><i className={segment.className} />{segment.label}</span>)}<span><i className="is-social-marker" />社保基数切换</span><span><i className="is-event-marker" />一次性事件</span></div>
      ...
      <details className="ledger-table-toggle"><summary>展开 12 个月明细表</summary><div className="ledger-table-wrap"><table><thead><tr><th>月份</th><th>工资</th><th>工资个税</th><th>消费内含税</th><th>奖金税</th><th>投资税</th><th>资产税</th><th>合计</th></tr></thead><tbody>{schedule.map((point) => <tr key={point.month}><th>{point.month} 月</th><td>{money(point.grossSalary)}</td><td>{money(point.salaryIncomeTax)}</td><td>{money(point.embeddedTax)}</td><td>{money(point.bonusTax)}</td><td>{money(point.investmentTax)}</td><td>{money(point.assetTax)}</td><td>{money(point.total)}</td></tr>)}</tbody></table></div></details>
    </div>
  )
}
  ...
  return (
    <div className="annual-dashboard">
      ...
      <section className="headline-ledger" aria-label="年度核心结果">
        ...
        <HeadlineMetric label="个人缴费（非税）" value={contributions} tone="social" note={`社保 ${money(result.personalSocial)} · 公积金 ${money(result.housingFund)}`} basis="社保缴费基数乘个人养老、医疗、失业比例，加住房公积金；不属于税。" />
