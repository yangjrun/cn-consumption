...
function AmountInput({ value, onChange, label }: { value: number; onChange: (value: number) => void; label: string }) {
  return (
    <label className="model-field">
      <span>{label}</span>
      <span className="model-money-input"><b>¥</b><input type="number" min="0" step="100" value={value} onChange={(event) => onChange(numeric(event.target.value))} /></span>
    </label>
  )
}
      ...
      <section className="model-editor" role="dialog" aria-modal="true" aria-labelledby="model-editor-title">
        <header className="model-editor__head">
          <div>
            ...
            <p>{onboarding ? '所有数据只保存在当前浏览器。完成后，你可以继续加入房车、投资或单笔消费事件。' : '修改会先留在草稿中，保存后才更新所有页面和年度报告。'}</p>
          </div>
          {!onboarding && <button className="icon-button" onClick={onClose}>关闭</button>}
        </header>

        <ol className="model-steps" aria-label="建模步骤">
          {steps.map((item, index) => (
            <li key={item.label} className={index === step ? 'is-active' : index < step ? 'is-done' : ''}>
              <button onClick={() => setStep(index)} aria-current={index === step ? 'step' : undefined}>
                <b>0{index + 1}</b><span>{item.label}<small>{item.hint}</small></span>
              </button>
            </li>
          ))}
        </ol>
        ...
        <div className="model-editor__body">
          {step === 0 && (
            <section className="model-step-panel">
              ...
              {showMonths && <div className="monthly-income-grid">{draft.monthlyIncome.map((item) => <label key={item.month}><span>{item.month} 月</span><input type="number" min="0" step="100" value={item.salary} onChange={(event) => patchMonth(item.month, numeric(event.target.value))} /></label>)}</div>}
        ...
        <footer className="model-editor__footer">
          <div>{onboarding && onSkip ? <button className="text-button" onClick={onSkip}>先使用示例数据</button> : <span>保存后所有页面同步更新</span>}</div>
          <div>
